...
def merge_img(src_img, dst_img, dst_matrix, dst_points, k_size=None, mat_multiple=None):
    ...
    for group in face_morph.OVERLAY_POINTS_EXPEND:
...
def morph_img_change(src_img, src_points, dst_img, dst_points, dt):
    
    face_shape_alpha = config.face_shape_alpha
    face_color_alpha = config.face_color_alpha
    ...
    for i in range(0, len(src_points)):
        x = (1 - face_shape_alpha) * src_points[i][0] + face_shape_alpha * dst_points[i][0]
        y = (1 - face_shape_alpha) * src_points[i][1] + face_shape_alpha * dst_points[i][1]
    ...
    for i in range(0, len(dt)):
        ...
        face_morph.morph_triangle(src_img, dst_img, res_img, t1, t2, t, face_color_alpha)
...
def face_change_merge(templet_img_align, templet_points, img_trans_aligned, trans_points, triangle_68_points, triangle_82_points):
    
    merge_two_img, morph_points_two = morph_img_change(templet_img_align, templet_points, img_trans_aligned, trans_points, triangle_68_points)
    ...
    whilte_face = face_morph.white_face(np.uint8(merge_two_img))
    result_img = merge_img(trans_templet_img, whilte_face, morp_matrix, morp_points, config.FaceSwitch_ker_size, config.FaceSwitch_mat_multiple)

    return result_img

def switching_face_interface(img,
        triangle_68_points,
        triangle_82_points,
        youth_name):

    code, img_aligned, _, img_aligned_points = face_process.face_detector.align_stable(img)
    if code != 201:
        return code, None, None

    img_matrix, img_points = get_matrix_and_box(img_aligned_points)

    templet_img_align, templet_img_align_points = face_match.get_change_baby(youth_name)
    ...
    result_imgs = []
    result_img = face_change_merge(templet_img_align, templet_points, img_trans_aligned, trans_points, triangle_68_points, triangle_82_points)
    result_imgs.append(result_img)
    
    return 202, result_imgs, img_aligned, templet_img_align