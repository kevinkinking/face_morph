...
def matrix_rectangle1(left, top, width, height):
    pointer = [
        (left, top),
        (left + width / 2, top),
        (left + width - 1, top),
        (left + width - 1, top + height / 10),
        (left, top + height / 10),
        (left, top + height - 1),
        (left + width / 2, top + height - 1),
        (left + width - 1, top + height - 1)
...
def morph_triangle(src, dst, img, t_src, t_dst, t, alpha):
    ...
    # k_size = (3, 3)
    # warp_img1 = cv2.blur(warp_img1, k_size, (r[2]/2, r[3]/2))
    # warp_img1 = cv2.blur(warp_img2, k_size, (r[2]/2, r[3]/2))
