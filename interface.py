...
@set_timeout(config.algo_timeout, after_timeout)
def face_switch_interface_url(img_url, choice_img_name):
    try:
        resp = urllib.urlopen(img_url)
        img_data_undecode = np.asarray(bytearray(resp.read()), dtype = 'uint8')
        img = cv2.imdecode(img_data_undecode, cv2.IMREAD_COLOR)
    ...
    code, result_imgs, _, _ = face_morph.switching_face_interface(img, triangle_68_points, triangle_82_points, choice_img_name)
    # print code
    urls = []
    if code == 202:
        result_imgs = img_util.corp_imgs_scale(result_imgs, 0.7)
...
if __name__ == '__main__':
    ...
    ##
    global_init_FaceSwitch()
    ti =timer.Timer()
    ti.tic()
    code, urls = face_switch_interface_url('data/test_img/7.jpg', '86')
    ti.toc()
    print ti.total_time
    print code
    print urls
